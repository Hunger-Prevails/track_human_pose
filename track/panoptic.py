...
def compare(cam_gt, cam_spec, args):
	...
	_rel_ = np.argsort(dist_rel, axis = -1)[:args.n_prime]  # (args.n_prime,)
	_cam_ = np.argsort(dist_cam, axis = -1)[:args.n_prime]  # (args.n_prime,)
...
def associate(cam_gt, cam_spec, args):
	...
	dist_rel = np.linalg.norm(rel_gt[valid] - rel_spec[:, valid], axis = -1)  # (n_det, n_valid)
	dist_cam = np.linalg.norm(cam_gt[valid] - cam_spec[:, valid], axis = -1)  # (n_det, n_valid)

	best_fit = np.argmin(np.sum(dist_rel + dist_cam, axis = -1))

	dist_rel = dist_rel[best_fit]  # (n_valid,)
	dist_cam = dist_cam[best_fit]  # (n_valid,)
...
class Dataset(data.Dataset):
	...
	def parse_sample(self, sample):
		...
		return track_feat, sample.cam_gt[:, :3] / 10.0, np.uint8(0.1 <= sample.cam_gt[:, 3])
